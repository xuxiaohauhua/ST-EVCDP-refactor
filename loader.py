...
def read_dataset(use_pca=True, pca_dims_inf=5, pca_dims_time=3):
    ...
    if use_pca:
        # 对information进行PCA处理，保留num列
        inf = process_information(inf, num_col="num", pca_components=pca_dims_inf)

        # 对time进行PCA处理
        time = process_time(time, pca_components=pca_dims_time)

        print("Information PCA shape:", inf.shape)
        print("Time PCA shape:", time.shape)

    return occ, prc, adj, col, dis, cap, time, inf

def apply_pca(data, n_components=5):   # apply PCA
...
def process_information(inf_df, num_col="num", pca_components=5):
    if num_col in inf_df.columns:
        num_col_data = inf_df[[num_col]]  # 保留num列
        other_cols_data = inf_df.drop(columns=[num_col])  # 去除num列
    else:
        num_col_data = None
        other_cols_data = inf_df

    inf_pca_df = apply_pca(other_cols_data, n_components=pca_components)
    if num_col_data is not None:
        inf_df_final = pd.concat([num_col_data.reset_index(drop=True), inf_pca_df], axis=1)
    else:
        inf_df_final = inf_pca_df

    return inf_df_final


def process_time(time_df, pca_components=3):
    time_num = pd.DataFrame({
        "year": time_df.dt.year,
        "month": time_df.dt.month,
        "day": time_df.dt.day,
        "hour": time_df.dt.hour,
        "minute": time_df.dt.minute,
        "second": time_df.dt.second
    })

    time_pca_df = apply_pca(time_num, n_components=pca_components)  # PCA降维
    return time_pca_df
# ---------data transform-----------
...
def metrics(test_pre, test_real): #评估
    eps = 0.01
